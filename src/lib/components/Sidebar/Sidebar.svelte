...
<div class="flex flex-col gap-5 bg-black p-4 lg:max-w-80 border-r border-purple-600">
  <div class="flex gap-1 rounded-full bg-slate-900 p-1 shadow-inner">
    {#each betModes as { value, label }}
      <button
        disabled={autoBetInterval !== null}
        onclick={() => (betMode = value)}
        class={twMerge(
          'flex-1 rounded-full py-2 text-sm font-medium text-white transition-all duration-200 hover:not-disabled:bg-slate-600 active:not-disabled:bg-slate-500 disabled:cursor-not-allowed disabled:opacity-50 shadow-sm',
          betMode === value && 'bg-gradient-to-r from-purple-600 to-purple-700 shadow-md',
        )}
      >
        {label}
      </button>
  ...
  <button
    ...
    class={twMerge(
      'touch-manipulation rounded-lg bg-gradient-to-r from-purple-500 to-purple-600 py-3 font-semibold text-white transition-all duration-200 hover:from-purple-400 hover:to-purple-500 active:from-purple-600 active:to-purple-700 disabled:from-neutral-600 disabled:to-neutral-700 disabled:text-neutral-400 shadow-lg hover:shadow-xl disabled:shadow-none',
      autoBetInterval !== null && 'from-purple-600 to-purple-700 hover:from-purple-500 hover:to-purple-600 active:from-purple-700 active:to-purple-800',
    )}
  >
    {#if betMode === BetMode.MANUAL}
      Drop Ball
